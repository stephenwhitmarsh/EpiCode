...
switch eventformat
  ...
  case {'neuralynx_nev'}
    ...
    if ~isempty(nev)
      ...
      eventstring = {nev.EventString}; %Paul
      % convert it into a structure array
      %event = struct('type', type, 'value', value, 'sample', sample, 'timestamp', timestamp, 'duration', duration, 'offset', offset, 'number', number);
      event = struct('type', type, 'value', value, 'sample', sample, 'timestamp', timestamp, 'duration', duration, 'offset', offset, 'number', number,'eventstring',eventstring);%Paul
    end
