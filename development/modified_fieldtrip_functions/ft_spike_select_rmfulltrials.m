  ...
  if doSelection
    ...
    for iUnit = 1:nUnits
      ...
      for itrial = cfg.trials' %Paul
          new_trialnr = new_trialnr + 1; %Paul
          spike.trial{iUnit}(spike.trial{iUnit}==itrial) = new_trialnr; %Paul
      end %Paul
    end
	
	%warning('No selection is performed on spike.trialtime and spike.sampleinfo'); %Paul
    %% Note: .trialtime and .sampleinfo are explicitly left intact, as the %Paul
    %% relationship between trial number and trialtime becomes ambigious %Paul
    %% otherwise! %Paul
