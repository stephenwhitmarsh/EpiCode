...
for idata = 1:size( analysis_names,2)
    ...
    for ichan= 1:numel(chan_list)%chan_name = string(fieldnames(data)')
        ...
        if idata > 12 & idata < 19
            config{4}.imagesavedir_data{1} = fullfile(config{4}.imagesavedir_data{3});
        end
        
        if idata >= 19
            config{4}.imagesavedir_data{1} = fullfile(config{4}.imagesavedir_data{3},'baseline_corrected');
        end
    ...
    if idata > 12 & idata < 19
        config{4}.imagesavedir_data{2} = fullfile(config{4}.imagesavedir_data{2},'Log');
    end
    
    if idata >= 19
        config{4}.imagesavedir_data{2} = fullfile(config{4}.imagesavedir_data{2},'Log','baseline_corrected');
    end
    ...
    if ~isfolder(config{4}.imagesavedir_data{4})
        fprintf('Creating directory %s\n', config{4}.imagesavedir);
        mkdir(config{4}.imagesavedir_data{4})
    end
    
    if idata >= 7 & idata <= 12
        config{4}.imagesavedir_data{4}=fullfile(config{4}.imagesavedir_data{4},'baseline_corrected');
    end
    
    if idata > 12 & idata < 19
        config{4}.imagesavedir_data{4} = fullfile(config{4}.imagesavedir_data{4},'Log');
    end
    
    if idata >= 19
        config{4}.imagesavedir_data{4} = fullfile(config{4}.imagesavedir_data{2},'Log','baseline_corrected');
    end
    ...
    set(fig,'PaperUnits','normalized');
    set(fig,'PaperPosition', [0 0 1 1]);
    print(fig, '-dpdf', fullfile(config{4}.imagesavedir_data{4},['AllRats_',analysis_names{idata},'.pdf']),'-r600');
    print(fig, '-dpng', fullfile(config{4}.imagesavedir_data{4},['AllRats_',analysis_names{idata},'.png']),'-r600');
    savefig(fig,fullfile(config{4}.imagesavedir_data{4},['AllRats_',analysis_names{idata}]));
    close all
    config{4}.imagesavedir_data{4}= fullfile(config{4}.imagesavedir,'LFHF_ratio');  
