function [SpikeRaw, SpikeTrials] = readSpykingCircus_allmarkers(cfg,MuseStruct)
...
fname = fullfile(cfg.datasavedir,[cfg.prefix,'all_concatinated_spikedata.mat']);
...
if exist(fname,'file') && cfg.force == false
    ...
    temp = dir(fullfile(cfg.datasavedir,'SpykingCircus',[cfg.prefix,'all_concatinated_',cfg.channel(1:end-2),'_*.result',cfg.suffix,'.hdf5']));
    fname_spikes = fullfile(temp.folder,temp.name);
    
    temp = dir(fullfile(cfg.datasavedir,'SpykingCircus',[cfg.prefix,'all_concatinated_',cfg.channel(1:end-2),'_*.templates',cfg.suffix,'.hdf5']));
    ...
    if exist(fname_spikes,'file')
        ...
        temp        = dir(fullfile(cfg.datasavedir,[cfg.prefix,'all_concatinated_',cfg.channel(1:end-2),'_*.ncs']));
...
%         timestamps  =  (0:hdr.nSamples-1) * hdr.TimeStampPerSample;
        ...
        for ilabel = 1 : size(cfg.label,2)
            ...
            for itemp = 1 : size(SpikeRaw.label,2)
                ...
                print(fig, '-dpdf', fullfile(cfg.imagesavedir,[cfg.prefix,'_',cfg.label{ilabel},'_raster_template_fromall_',num2str(itemp),'.pdf']),'-r600');
                print(fig, '-dpng', fullfile(cfg.imagesavedir,[cfg.prefix,'_',cfg.label{ilabel},'_raster_template_fromall_',num2str(itemp),'.png']),'-r600');
...
%                 set(gca, 'XTickLabel', '');
            ...
            set(fig,'PaperUnits','normalized');
            set(fig,'PaperPosition', [0 0 1 1]);
            print(fig, '-dpdf', fullfile(cfg.imagesavedir,[cfg.prefix,'_',cfg.label{ilabel},'_all_concatinated_templates_morphology.pdf']));
            print(fig, '-dpng', fullfile(cfg.imagesavedir,[cfg.prefix,'_',cfg.label{ilabel},'_all_concatinated_templates_morphology.png']));
            ...
            set(fig,'PaperUnits','normalized');
            set(fig,'PaperPosition', [0 0 1 1]);
            print(fig, '-dpdf', fullfile(cfg.imagesavedir,[cfg.prefix,'_',cfg.label{ilabel},'_raster_all_concatinated_templates_fromall.pdf']));
            print(fig, '-dpng', fullfile(cfg.imagesavedir,[cfg.prefix,'_',cfg.label{ilabel},'_raster_all_concatinated_templates_fromall.png']));
