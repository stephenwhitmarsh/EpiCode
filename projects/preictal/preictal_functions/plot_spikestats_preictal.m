...
for ipart = 1:size(spikedata,2)
    for ilabel = 1:size(stats{ipart}, 2)
        ...
        for i_unit = 1:size(spikedata{ipart}{ilabel}.label, 2)
            ...
            %% prepare figure
            fig = figure;
            phy_channr      = spikedata{ipart}{ilabel}.template_maxchan(i_unit);
            nlx_channame    = cfg.circus.channel{phy_channr+1}; %+1 because phy_channr starts at zero
            sgtitle(sprintf('Electrode n°%d %s : %s %s',phy_channr,nlx_channame, spikedata{ipart}{ilabel}.label{i_unit}, cfg.statstime.plot.suffix), 'Fontsize', 22, 'Interpreter', 'none', 'FontWeight', 'bold');
            
          
            
            %% plot ISI
            subplot(4,4,2);hold;
            stats{ipart}{ilabel}.isi{i_unit} = diff(double(spikedata{ipart}{ilabel}.samples{i_unit})) / spikedata{ipart}{ilabel}.hdr.Fs ;
            ...
            disp_text = sprintf('%d spikes \n\nMean values on all the data : \n- Amplitude = %.2f µV \n- RPV = %.2f %% \n- Freq = %.2f Hz \n- CV2 = %.2f \n- Fano Factor = %.2f \n- CV = %.2f \n- Burst index = %.2f', size(spikedata{ipart}{ilabel}.samples{i_unit},2), meanamplitude, RPV, meanfreq, meancv2, meanff, meancv, meanbi);
            ...
            %% plot template
            subplot(4,4,3);hold;
            tempsel = spikedata{ipart}{ilabel}.template{i_unit}(:,spikedata{ipart}{ilabel}.template_maxchan(i_unit)+1,:);%+1 because electrodes nr are zero-based
            temptime = ( (0 : size(spikedata{ipart}{ilabel}.template{i_unit},3) - 1) / spikedata{ipart}{ilabel}.hdr.Fs )';
            ...
            for ivalue = ["freq","amplitude","cv", "cv2", "fanofactor", "burstindex"]
                ...
                for imarker = 1:size(cfg.statstime.plot.marker2,2)
                    plot([cfg.statstime.plot.marker2(imarker) cfg.statstime.plot.marker2(imarker)], [ax(3) ax(4)], 'r');
                end
            ...
            print(fig, '-dpdf', fullfile(cfg.imagesavedir,[cfg.prefix,'p',num2str(ipart),'-',spikedata{ipart}{ilabel}.label{i_unit},'_spikestatsOverTime_',cfg.statstime.plot.suffix,'.pdf']),'-r600');
            print(fig, '-dpng', fullfile(cfg.imagesavedir,[cfg.prefix,'p',num2str(ipart),'-',spikedata{ipart}{ilabel}.label{i_unit},'_spikestatsOverTime_',cfg.statstime.plot.suffix,'.png']),'-r600');
            close all
