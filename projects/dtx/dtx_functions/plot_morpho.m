...
%         try %REMOVEME FIXME
        ...
        %plot(data_avg_bl.time,data_avg_bl.avg.*flip)
        %plot(data_avg_ac.time,data_avg_ac.avg.*flip)
        ...
        elseif all(x_intersect <=0) || all(x_intersect >=0)
            ...
            p = plot(x_intersect(indx),y_intersect(indx).*flip,'-x','Color','g','MarkerFaceColor','g','MarkerEdgeColor','g');
...
%         end
        ...
        %If found at least 2 values, check first than they are not at the
        %same side of zero
...
if strcmp(cfg.morpho.measureamplitude, 'yes')
    ...
    %scatter(amplitude_loc, peak.*flip, 'xb');
    %scatter(amplitude_loc, bl.*flip, 'xb');
