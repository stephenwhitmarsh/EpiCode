...
for itemp = 1:size(SpikeStats_windowed{ipart}.window, 2)
    ...
    neurons_table.is_hr{itemp}                      = "";
    neurons_table.stab_freq{itemp}                  = "";
    neurons_table.stab_amp{itemp}                   = "";
    neurons_table.select_time{itemp}                = "";
    neurons_table.waveform_qual{itemp}              = "";
    neurons_table.merge{itemp}                      = "";
    if istimelocked
        neurons_table.sw_maxfreq{itemp}             = nanmax(SpikeDensity_timelocked{ipart}.sdf_lin.SlowWave.avg(itemp, :));
        neurons_table.sw_minfreq{itemp}             = nanmin(SpikeDensity_timelocked{ipart}.sdf_lin.SlowWave.avg(itemp, :));
        neurons_table.sw_behavior{itemp}            = "";
        neurons_table.sw_code{itemp}                = "";
        neurons_table.interictal_behavior{itemp}    = "";
        neurons_table.interctal_code{itemp}         = "";
    end
end
