function plotOverviewDTX(cfg,SpikeTrials_timelocked, SpikeTrials_windowed, SpikeStats_windowed, SpikeDensity_timelocked, LFP, TFR, SpikeWaveforms, SpikeWaveforms_stats, MuseStruct_concat, seizure_infos)
...
%% go trhough each part and each units
for ipart = 1:size(SpikeTrials_windowed,2)
    ...
    for itemp = 1:size(SpikeTrials_windowed{ipart}.window.label, 2)
        ...
        fig = figure;
        unit_name     = SpikeTrials_windowed{ipart}.window.label{itemp};
        chanoffset    = find(strcmp(SpikeTrials_windowed{ipart}.window.channelname(itemp),cfg.circus.channelname), 1, 'first');
        maxchan       = SpikeTrials_windowed{ipart}.window.template_maxchan(itemp) + chanoffset;
        channel       = cfg.circus.channelname{maxchan};
        chan_deepness = (str2double(unit_name(end-1:end)) - 16) * 250;%16 because probe with 16 electrodes
        sgtitle_obj   = sgtitle(sprintf('%s (%guM)', unit_name, chan_deepness), 'Interpreter', 'none', 'FontWeight', 'bold', 'Fontsize', 22);
        ...
        %% analysis only for dtx, and not control :
        if istimelocked
            %% Event LFP & TFR
...
%             clear y
        ...
        channame  = temp{3};
        fname = fullfile(cfg.imagesavedir,'spike_overview',sprintf('%sp%d-%s_%s_overview_%s_%s', cfg.prefix, ipart,channame, temp_name, char(event_name), char(baseline_name)));
