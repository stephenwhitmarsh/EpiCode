...
if isfield(cfg.seizuretimings, 'marker_end')
    time_end   = MuseStruct_concat{ipart}.markers.(cfg.seizuretimings.marker_end);%;%concatenateMuseMarker(cfg,MuseStruct_concat,ipart, cfg.seizuretimings.marker_end);
else
    time_end = time_start;
end
...
arraytemp = seizure_infos.timebetween2seizures;
cv2_data = NaN;
...
if isfield(cfg.seizuretimings, 'marker_end')
    arraytemp = seizure_infos.seizureduration;
    cv2_data = NaN;
    if length(arraytemp)>2
        for i = 1:length(arraytemp)-1
            cv2_data(i) = 2*abs(arraytemp(i)-arraytemp(i+1))/(arraytemp(i)+arraytemp(i+1));
...
%% order fields to be more readable, then save to disk
