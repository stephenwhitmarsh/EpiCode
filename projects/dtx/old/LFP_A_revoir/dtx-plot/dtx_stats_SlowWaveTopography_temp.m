...
cfgtemp.latency                         = 'all';%cfg.align.toiactive{1}; % active period
    ...
    if isfield(stat,'posclusters')
        for ipos = 1 : size(stat.posclusters,2)
            if stat.posclusters(ipos).prob < cfg.stats.alpha
                ...
                plot(stat.time(sel),dat_EEG_avg.avg(ichan,round(sel))+(nb_channels+1)*h-h*h_amplitude,'.g','linewidth',2); % +bl index to begin in the active period if any (here is 'all' so no need)
            end
        end
    end
    ...
    if isfield(stat,'negclusters')
        for ineg = 1 : size(stat.negclusters,2)
            if stat.negclusters(ineg).prob < cfg.stats.alpha
                ...
                plot(stat.time(sel),dat_EEG_avg.avg(ichan,round(sel))+(nb_channels+1)*h-h*h_amplitude,'.r','linewidth',2);
            end
        end
    end
end %ichan
...
t1_activeperiod_idx             = find(dat_EEG_avg.time > cfg.stats.toilatency(1),1,'first');
t2_activeperiod_idx             = find(dat_EEG_avg.time < cfg.stats.toilatency(2),1,'last');
