function [MuseStruct] = alignMuseMarkers_begin(cfg,MuseStruct, force)
...
fname = fullfile(cfg.datasavedir,[cfg.prefix,'MuseStruct_aligned_thresh.mat']);
...
elseif exist(fname,'file') && force == true
    ...
    for i = 1 : size(cfg.name,2)
        if ismember(cfg.name{i}, cfg.align.begin.name)
            markerlist = [markerlist, i];
        end
    end
    
    if isempty(markerlist)
        fprintf('There is no marker to align with alignMuseMarkers_begin.m\n');
        return
    end
    ...
    for ipart = 1 : size(cfg.directorylist,2)
        ...
        for imarker = markerlist
            ...
            for idir = markerindx
                ...
                %% find begining of event according to % of peak
                 for itrial = 1 : size(dat_filt_trl.trial,2)
                ...
                for itrial = 1 : size(dat_filt_trl.trial,2)
                    if haspeak(itrial)
                        ...
                        thresh(itrial)          = (peaks_ac_sel{itrial}(ip(itrial)) - med_all_bl) * cfg.align.begin.thresh;
                        index_tresh{itrial}     = zci(dat_filt_trl.trial{itrial}(t1_ac_indx(itrial):peak_loc(itrial)) - med_all_bl - thresh(itrial)) + t1_ac_indx(itrial) - 1;
                        
                        if ~isempty(index_tresh{itrial})
                            index_tresh{itrial}             = index_tresh{itrial}(end-1); %take last good index (-1 because last = last of trial)
                            ...
                            if abs(timeshift) > cfg.align.maxtimeshift || timeshift > 0
                                hasartefact(itrial) = true;
                            end 

                            %create new MuseStruct events
                            if ~hasartefact
                ...
                if ~isfolder(fullfile(cfg.imagesavedir,'alignment_tresh'))
                    ft_notice('creating directory %s', fullfile(cfg.imagesavedir,'alignment_tresh'));
                    mkdir(fullfile(cfg.imagesavedir,'alignment_tresh'));
                end
                ...
                print(fig, '-dpdf', fullfile(cfg.imagesavedir,'alignment_tresh',[cfg.prefix,'p',num2str(ipart),'_',cfg.name{imarker},'_',dat_sel_aligned.label{1},'_',num2str(idir),'.pdf']));
                print(fig, '-dpng', fullfile(cfg.imagesavedir,'alignment_tresh',[cfg.prefix,'p',num2str(ipart),'_',cfg.name{imarker},'_',dat_sel_aligned.label{1},'_',num2str(idir),'.png']),'-r600');
    ...
    % save data
    save(fullfile(cfg.datasavedir,[cfg.prefix,'MuseStruct_aligned_begin']),'MuseStruct');
    save(fullfile(cfg.datasavedir,[cfg.prefix,'MuseStruct_aligned_begin']),'MuseStruct');
end
