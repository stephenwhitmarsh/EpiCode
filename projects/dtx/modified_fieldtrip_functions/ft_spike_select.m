...
if ~doAll
  ...
  try, spike.template = spike.template(spikesel); end
  try, spike.template_maxchan = spike.template_maxchan(spikesel); end
  try, spike.amplitude = spike.amplitude(spikesel); end
  try, spike.samples = spike.samples(spikesel); end
  try, spike.cluster_group = spike.cluster_group(spikesel); end
end
...
if ~isfield(spike,'trial') || ~isfield(spike,'trialtime') || ~isfield(spike,'time')
  ...
  if doSelection
    ...
    for iUnit = 1:nUnits
      ...
      try, spike.amplitude{iUnit} = spike.amplitude{iUnit}(spikesInTrial,:,:); end
      try, spike.samples{iUnit} = spike.samples{iUnit}(spikesInTrial,:,:); end
      %reorder trials from 1
      new_trialnr = 0;
      for itrial = cfg.trials'
          new_trialnr = new_trialnr + 1;
          spike.trial{iUnit}(spike.trial{iUnit}==itrial) = new_trialnr;
      end
    end
