...
% cfg.keepindexes    : 'yes' or 'no', whether to keep indexes of removed
%                       trials in MuseStruct. This option use cfg.muse.startend,
...
%                      readLFP.m and readSpykingCircus.m. Can be obtained
%                      by another function if the trialinfo in consistent.
        ...
        if ~isempty(data{ipart}{ilabel})
            ...
            if ~isempty(data{ipart}{ilabel})
                ...
                for idir = 1:length(MuseStruct{ipart})
                            ...
                            if size(MuseStruct{ipart}{idir}.markers.(cfg.markerstart).synctime,2)-size(MuseStruct{ipart}{idir}.markers.(cfg.markerend).synctime,2) == 0
                                ...
                                for itrial = trials_idx
                                    ...
                                    for isearchedinterval = 1 : size(MuseStruct{ipart}{idir}.markers.(cfg.markerstart).synctime,2)
                                        ...
                                        if isearchedinterval + cfg.indexstart <= length(MuseStruct{ipart}{idir}.markers.(cfg.markerstart).synctime)
                                            if isearchedinterval + cfg.indexend <= length(MuseStruct{ipart}{idir}.markers.(cfg.markerend).synctime)
                                            ...
                                            if intersect(trialinterval,searchedinterval) %intervals are expressed in sample points
                                                ...
                                                if strcmp(cfg.keeptrials, 'yes') %keep indexes of removed trials
                    ...
                    if strcmp(cfg.method,'keep')&&strcmp(cfg.keeptrials, 'yes')
                ...
                if strcmp(cfg.plotdata, 'yes') && strcmp(type, 'raw')
                    ...
                    for idir = unique(data_plot.trialinfo(:,dircolumn))'
                        ...
                        for itrial = 1:size(trialremoved{idir},2)
                        ...
                        if nr_notloaded(idir) == 0
                            title(sprintf('%s chan %s \nAll trials are loaded \n%d searched-trials removed from %d trials \nSearch between %s(i+%d)%+gs and %s(i+%d)%+gs', cfg.LFP.name{ilabel}, data_plot.label{1}, sum(trialremoved{idir}), size(trialremoved{idir},2), cfg.markerstart,cfg.indexstart, cfg.timefrombegin, cfg.markerend,cfg.indexend, cfg.timefromend),'Fontsize',20,'Interpreter','none');
                        else
                            title(sprintf('%s chan %s \n%d trials are not loaded (too close from begin or end of data) \n%d searched-trials trials removed from %d trials\nSearch between %s(i+%d)%+gs and %s(i+%d)%+gs', cfg.LFP.name{ilabel}, data_plot.label{1}, nr_notloaded(idir), sum(trialremoved{idir}), size(trialremoved{idir},2), cfg.markerstart,cfg.indexstart, cfg.timefrombegin, cfg.markerend,cfg.indexend, cfg.timefromend),'Fontsize',20,'Interpreter','none');
                        end
