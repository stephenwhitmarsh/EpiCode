        ...
        for idir = markerindx
            ...
            switch cfg.align.filter.(markername)
                ...
                case 'bp'
                    ...
                    cfgtemp.bpfreq              = cfg.align.freq.(markername);
                    cfgtemp.bpfilttype          = 'but';
                    cfgtemp.bpinstabilityfix    = 'reduce';
                    dat_filt                    = ft_preprocessing(cfgtemp,dat_sel);
            ...
            toi(1) = min([cfg.align.toiplot.(markername), cfg.align.toibaseline.(markername), cfg.align.toiactive.(markername)]);
            toi(2) = max([cfg.align.toiplot.(markername), cfg.align.toibaseline.(markername), cfg.align.toiactive.(markername)]);
            startsample         = round(MuseStruct{ipart}{idir}.markers.(cfg.muse.startmarker.(markername)).synctime*dat_sel.fsample + toi(1)*dat_sel.fsample + hdr.nSamplesPre)';
            endsample           = round(MuseStruct{ipart}{idir}.markers.(cfg.muse.endmarker.(markername)).synctime*dat_sel.fsample + toi(2)*dat_sel.fsample + hdr.nSamplesPre)';
            offset              = round(ones(size(startsample))*+toi(1)*dat_sel.fsample);
            cfgtemp             = [];
