...
if exist(fname,'file') && force == false
    ...
    for imarker = 1 : size(cfg.name,2)
        ...
        for idir = markerindx
            ...
            % flip
            if strcmp(cfg.align.method{imarker},'min')
            ...
            cfgtemp             = [];
            cfgtemp.trl         = [MuseStruct_micro{idir}.markers.(cfg.muse.startend{imarker,1}).offset'+cfg.align.toibaseline{imarker}(1)*dat_filt.fsample, ...
                                   MuseStruct_micro{idir}.markers.(cfg.muse.startend{imarker,1}).offset'+cfg.align.toiactive{imarker}(2)*dat_filt.fsample];
            cfgtemp.trl(:,3)    = ones(size(cfgtemp.trl,1),1)*+cfg.align.toibaseline{imarker}(1)*dat_filt.fsample;
            ...
            for itrial = 1 : size(dat_filt_trl.trial,2)
                if haspeak(itrial)
                    ...
                    elseif strcmp(cfg.align.method{imarker},'min') % Same as max because it was flipped!
                        [~, ip(itrial)] = max(dat_filt_trl.trial{itrial}(locs_ac_sel_avg{itrial}+t1_ac_indx(itrial)-1)); % min peak
