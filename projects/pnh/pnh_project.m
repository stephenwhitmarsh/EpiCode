    ...
    for ipattern = 1 : size(SpikeRateStats{ipatient}.isi_pattern_all,2)
        ...
        for itemp = 1 : length(SpikeRateStats{ipatient}.isi_pattern_all{ipattern}.isi)
            ...
            for itrial                 = unique(SpikeRateStats{ipatient}.isi_pattern_bl{ipattern}.trial{itemp})
                ...
                trialavg_isi_bl(i)     = nanmean(SpikeRateStats{ipatient}.isi_pattern_bl{ipattern}.isi{itemp}(trialindx));
                i = i + 1;
            end
            i = 1;
...
filename = fullfile(config{ipatient}.datasavedir,'spikestats_new_again_again.xlsx');
writetable(tbl,filename)
filename = fullfile(config{ipatient}.datasavedir,'spikestats.csv');
writetable(tbl,filename)
