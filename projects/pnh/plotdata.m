function plotdata(cfg,dat_micro,dat_macro,SpikeTrials)
...
subplot(6,2,7); hold;
[~,~,W,P] = findpeaks(dat_micro_rptavg.avg,dat_micro_rptavg.time,'MinPeakProminence',4,'Annotate','extents');
findpeaks(dat_micro_rptavg.avg,dat_micro_rptavg.time,'MinPeakProminence',4,'Annotate','extents');
legend('off');
ylabel('Microvolts');
[~, ci] = max(P);
...
% sdf                     = ft_spikedensity(cfgtemp,SpikeTrials{2});
% 
% cfgtemp                 = [];
% cfgtemp.topplotfunc     = 'line'; % plot as a line
% cfgtemp.latency         = [-cfg.prestim, cfg.poststim];
% cfgtemp.spikechannel    = 'all';
% cfgtemp.errorbars       = 'no'; % plot with the standard deviation
% cfgtemp.interactive     = 'no'; % toggle off interactive mode
% ft_spike_plot_raster(cfgtemp,SpikeTrials{2}, sdf) 
...
if exist(fname,'file') && cfg.force == false
    load(fname,'TFR_micro_singletrial');
else
...
%     cfgtemp.t_ftimwin       = ones(size(cfgtemp.foi))*0.5;
    ...
    save(fname,'TFR_micro_singletrial','-v7.3');
...
% figure;
% w = ceil(sqrt(size(SpikeTrials{2}.label,2)));
% time = ((1 : size(SpikeTrials{2}.template,2)) - size(SpikeTrials{2}.template,2)/2) / SpikeTrials{2}.cfg.hdr.Fs;
% SpikeTrials{2}.cfg.hdr.Fs 
% for itemp = 1 : size(SpikeTrials{2}.label,2)
%     subplot(w,w,itemp);
%     plot(time,SpikeTrials{2}.template(itemp,:));
%     title(num2str(SpikeTrials{2}.tempchan(itemp)));
% end
% 
% % print to file
% set(fig,'PaperOrientation','landscape');
% set(fig,'PaperUnits','normalized');
% set(fig,'PaperPosition', [0 0 1 1]);
% print(fig, '-dpdf', fullfile(cfg.imagesavedir,'spike_templates.pdf'),'-r600');
