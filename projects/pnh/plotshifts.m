...
ncol = max(SpikeTrials_timelocked{ipatient}{1}.ES.trialinfo.idir);

for idir = unique(SpikeTrials_timelocked{ipatient}{1}.ES.trialinfo.idir)'
    
    cfgtemp = [];
    cfgtemp.trials = LFP{ipatient}{1}.ES.trialinfo.idir == idir;
    cfgtemp.avgoverrpt = 'yes';
    sel = ft_selectdata(cfgtemp, LFP{ipatient}{1}.ES);
    subplot(2, ncol, idir);
    ...
    cfg_raster.trials           = SpikeTrials_timelocked{ipatient}{1}.ES.trialinfo.idir == idir;
    
    subplot(2, ncol, idir + ncol);
    
    ft_spike_plot_raster(cfg_raster, SpikeTrials_timelocked{ipatient}{1}.ES);
    hold on
    plot([0, 0], [ax(3), ax(4)], ':k');
