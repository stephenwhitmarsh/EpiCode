function [artefacts] = writeSpykingCircus_deadfiles(cfg, MuseStruct, force)
...
for ipart = 1 : size(cfg.directorylist, 2)
    ...
    if isempty(cfg.circus.channelname)  
        ...
        for chandir = unique(cfg.circus.channelname)
            filename = 'SpykingCircus_artefacts_ms.dead';
            fprintf('Writing artefacts for Spyking-Circus to: %s\n', filename);
            dlmwrite(fullfile(cfg.datasavedir, subjdir, partdir, chandir, filename), deadfile_ms, 'delimiter', '	', 'precision', '%.4f');
            
            filename = 'SpykingCircus_artefacts_samples.dead';
            fprintf('Writing artefacts for Spyking-Circus to: %s\n', filename);
            dlmwrite(fullfile(cfg.datasavedir, subjdir, partdir, chandir, filename), deadfile_samples, 'delimiter', '	', 'precision', '%.4f');
        end
    end
