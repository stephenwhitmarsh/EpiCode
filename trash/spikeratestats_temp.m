...
function [stats] = spikeratestats(cfg,SpikeRaw,SpikeTrials,force)
...
if exist(fname,'file') && force == false
    ...
    for ilabel = 1 : size(SpikeTrials,2)
       ...
       % plot firing rate as line
            ...
            % plot positive clusters
            lag = find(sdf_bar.time < cfgtemp.latency(1),1,'last');
            ...
            x = (Xpos + Xneg(1))/2;
            y = Yneg(1)*0.1;
            text(x,y,sprintf('%.2fms',abs(Xpos-Xneg(1))),'HorizontalAlignment','center');
            
            x = (Xpos + Xneg(2))/2;
            y = -Yneg(2)*0.1;
            text(x,y,sprintf('%.2fms',abs(Xpos-Xneg(2))),'HorizontalAlignment','center');
            xlabel('time')
            ...
            y = midline*1.1;
            text(x,y,sprintf('%.2fms',temptime_int(indx(2))-temptime_int(indx(1))),'HorizontalAlignment','center');
