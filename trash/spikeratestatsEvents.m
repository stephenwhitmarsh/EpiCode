...
if exist(fname, 'file') && force == false
    ...
    for ipart = cfg.spike.part_list
        ...
        for itemp = 1 : nrtemplates
            stats_smooth{ipart}.isi{itemp} = diff(double(SpikeRaw{ipart}.sample{itemp})) / hdr.Fs * 1000;
        end
        ...
        for markername = string(cfg.name)
            ...
            stats_smooth{ipart}.(markername).isi_all = ft_spike_isi(cfgtemp, SpikeTrials{ipart}.(markername));
            
            cfgtemp.latency                          = cfg.stats.(markername).bl;
            stats_smooth{ipart}.(markername).isi_bl  = ft_spike_isi(cfgtemp, SpikeTrials{ipart}.(markername));
            
            cfgtemp.latency                          = cfg.stats.(markername).toi;
            stats_smooth{ipart}.(markername).isi_ac  = ft_spike_isi(cfgtemp, SpikeTrials{ipart}.(markername));
            ...
            for itemp = 1 : nrtemplates
                ...
                bar(stats_smooth{ipart}.(markername).isi_all.time*1000, stats_smooth{ipart}.(markername).isi_all.avg(itemp, :), 1);
                [y, indx] = max(stats_smooth{ipart}.(markername).isi_all.avg(itemp, :));
                title(sprintf('Unit: %d, Max ISI: all %.1fms', itemp, stats_smooth{ipart}.(markername).isi_all.time(indx)*1000));
                xlabel('ms');
                xticks(stats_smooth{ipart}.(markername).isi_all.time * 1000);
                xtickangle(90);
                axis tight
                grid on
                set(gca, 'fontsize', 6);
                ...
                bar(stats_smooth{ipart}.(markername).isi_bl.time*1000, stats_smooth{ipart}.(markername).isi_bl.avg(itemp, :), 1);
                [y, indx] = max(stats_smooth{ipart}.(markername).isi_bl.avg(itemp, :));
                title(sprintf('Unit: %d, Max ISI baseline: %.1fms', itemp, stats_smooth{ipart}.(markername).isi_bl.time(indx)*1000));
                xlabel('ms');
                xticks(stats_smooth{ipart}.(markername).isi_bl.time*1000);
                xtickangle(90);
                axis tight
                grid on
                set(gca, 'fontsize', 6);
                ...
                bar(stats_smooth{ipart}.(markername).isi_ac.time*1000, stats_smooth{ipart}.(markername).isi_ac.avg(itemp, :), 1);
                [y, indx] = max(stats_smooth{ipart}.(markername).isi_ac.avg(itemp, :));
                title(sprintf('Unit: %d, Max ISI active: %.1fms', itemp, stats_smooth{ipart}.(markername).isi_ac.time(indx)*1000));
                xlabel('ms');
                xticks(stats_smooth{ipart}.(markername).isi_ac.time*1000);
                xtickangle(90);
                axis tight
                grid on
                set(gca, 'fontsize', 6);
            ...
            set(fig, 'PaperUnits', 'normalized');
            set(fig, 'PaperPosition', [0 0 1 1]);
            print(fig, '-dpdf', fullfile(cfg.imagesavedir, strcat(cfg.prefix, 'ISI_', markername, '.pdf')), '-r600');

%             % spike density function, with smoothed version
%             cfgtemp                         = [];
%             cfgtemp.fsample                 = cfg.spike.resamplefs;   % sample at 1000 hz
%             cfgtemp.keeptrials              = 'yes';
%             cfgtemp.latency                 = [cfg.spike.(markername).toi(1), cfg.spike.(markername).toi(2)];
...
%             sdf_smooth{ipart}               = ft_spikedensity(cfgtemp, SpikeTrials{ipart}.(markername));
% 
...
%                 cfgtemp.latency                                     = [cfg.stats.(markername).bl(2) sdf_orig{ipart}.time(end)]; % active period starts after baseline
%                 cfgtemp.ivar                                        = 1;
%                 cfgtemp.uvar                                        = 2;
...
%                 stats_smooth{ipart}.(markername).clusterstat{itemp} = ft_timelockstatistics(cfgtemp, sdf_smooth{ipart}, sdf_bl{ipart});
% 
%                 % calculate baseline
%                 slim(1) = find(sdf_orig{ipart}.time > cfg.stats.(markername).bl(1), 1, 'first');
%                 slim(2) = find(sdf_orig{ipart}.time < cfg.stats.(markername).bl(2), 1, 'last');
%                 stats_smooth{ipart}.(markername).clusterstat{itemp}.bl.avg        = nanmean(sdf_orig{ipart}.avg(itemp, slim(1):slim(2)), 2);
%                 stats_smooth{ipart}.(markername).clusterstat{itemp}.bl.var        = nanmean(sdf_orig{ipart}.var(itemp, slim(1):slim(2)), 2);
%                 stats_smooth{ipart}.(markername).clusterstat{itemp}.bl.dof        = nanmean(sdf_orig{ipart}.dof(itemp, slim(1):slim(2)), 2);
%                 stats_smooth{ipart}.(markername).clusterstat{itemp}.bl.trialavg   = nanmean(sdf_orig{ipart}.trial(:, itemp, slim(1):slim(2)), 3);
%             end
            ...
            for itemp = 1 : nrtemplates
                ...
                lag = -(sdf_orig{ipart}.time(1) - cfg.stats.(markername).bl(2)) * cfg.spike.resamplefs;

                if isfield(stats_smooth{ipart}.(markername).clusterstat{itemp}, 'posclusters')
                    for ipos = 1 : size(stats_smooth{ipart}.(markername).clusterstat{itemp}.posclusters, 2)
                        if stats_smooth{ipart}.(markername).clusterstat{itemp}.posclusters(ipos).prob < cfg.stats.alpha
                            sel     = find(stats_smooth{ipart}.(markername).clusterstat{itemp}.posclusterslabelmat == ipos);
                            plot(stats_smooth{ipart}.(markername).clusterstat{itemp}.time(sel), sdf_smooth{ipart}.avg(itemp, round(sel+lag)), 'g', 'linewidth', 2); % smoothed
                            si      = round(sel+lag);
                            ...
                            d       = (sdf_smooth{ipart}.avg(itemp, si(I)) / stats_smooth{ipart}.(markername).clusterstat{itemp}.bl.avg) * 100;
                ...
                if isfield(stats_smooth{ipart}.(markername).clusterstat{itemp}, 'negclusters')
                    for ineg = 1 : size(stats_smooth{ipart}.(markername).clusterstat{itemp}.negclusters, 2)
                        if stats_smooth{ipart}.(markername).clusterstat{itemp}.negclusters(ineg).prob < cfg.stats.alpha
                            sel = find(stats_smooth{ipart}.(markername).clusterstat{itemp}.negclusterslabelmat == ineg);
                            plot(stats_smooth{ipart}.(markername).clusterstat{itemp}.time(sel), sdf_smooth{ipart}.avg(itemp, round(sel+lag+0.5)), 'r', 'linewidth', 2); % smoothed
                            si = round(sel+lag);
                ...
                % plot baseline
                plot(cfg.spike.(markername).toi, [stats_smooth{ipart}.(markername).clusterstat{itemp}.bl.avg, stats_smooth{ipart}.(markername).clusterstat{itemp}.bl.avg], ':k');

                % determine zero crossing
%                 zci             = @(v) find(v(:).*circshift(v(:), [-1 0]) <= 0);
%                 indx            = zci(sdf_smooth{ipart}.avg(itemp, :) - stats_smooth{ipart}.(markername).clusterstat{itemp}.bl.avg);
...
% 
                ...
                % plot baseline patch
                x = [cfg.stats.(markername).bl(1) cfg.stats.(markername).bl(2) cfg.stats.(markername).bl(2) cfg.stats.(markername).bl(1)];
                ax = axis;
                y = [ax(3) ax(3) ax(4) ax(4)];
                patch('XData', x, 'YData', y, 'facecolor', [0 0 0], 'edgecolor', 'none', 'facealpha', 0.1);

                % plot baseline text
                x = (cfg.stats.(markername).bl(1) + cfg.stats.(markername).bl(2))/2;
                y = ax(4)*0.9;
                d = stats_smooth{ipart}.(markername).clusterstat{itemp}.bl.avg;
                ...
                sdf_bar_out{ipart}.(markername)     = sdf_bar{ipart};
                sdf_orig_out{ipart}.(markername)    = sdf_orig{ipart};

                % calculate baseline for dummy stats
                slim(1)                             = find(sdf_bar{ipart}.time > cfg.stats.(markername).bl(1), 1, 'first');
                slim(2)                             = find(sdf_bar{ipart}.time < cfg.stats.(markername).bl(2), 1, 'last');
                ...
                cfgtemp.computestat                 = 'yes';
                cfgtemp.correctm                    = 'cluster';
                cfgtemp.latency                     = [cfg.stats.(markername).bl(2) sdf_orig{ipart}.time(end)]; % active perio starts after baseline
                cfgtemp.ivar                        = 1;
                cfgtemp.uvar                        = 2;
                ...
                cfgtemp.numrandomization            = 100;
                cfgtemp.channel                     = 1;
                stats_binned{ipart}.(markername).clusterstat{itemp} = ft_timelockstatistics(cfgtemp, sdf_bar{ipart}, sdf_bar_bl{ipart});

                % calculate baseline
                slim(1) = find(sdf_bar{ipart}.time > cfg.stats.(markername).bl(1), 1, 'first');
                slim(2) = find(sdf_bar{ipart}.time < cfg.stats.(markername).bl(2), 1, 'last');
                stats_binned{ipart}.(markername).clusterstat{itemp}.bl.avg        = nanmean(sdf_bar{ipart}.avg(slim(1):slim(2)));
                stats_binned{ipart}.(markername).clusterstat{itemp}.bl.trialavg   = nanmean(sdf_bar{ipart}.trial(:, 1, slim(1):slim(2)), 3);
                ...
                % plot positive clusters
                lag = size(sdf_bar{ipart}.avg, 1) - size(stats_binned{ipart}.(markername).clusterstat{itemp}.mask, 2);

                if isfield(stats_binned{ipart}.(markername).clusterstat{itemp}, 'posclusters')
                    for ipos = 1 : size(stats_binned{ipart}.(markername).clusterstat{itemp}.posclusters, 2)
                        if stats_binned{ipart}.(markername).clusterstat{itemp}.posclusters(ipos).prob < cfg.stats.alpha
                            sel = find(stats_binned{ipart}.(markername).clusterstat{itemp}.posclusterslabelmat == ipos);
                            bar(stats_binned{ipart}.(markername).clusterstat{itemp}.time(sel), sdf_bar{ipart}.avg(sel+lag), 1, 'facecolor', 'g', 'edgecolor', 'g');
                            ...
                            d = (sdf_bar{ipart}.avg(si(I)) / stats_binned{ipart}.(markername).clusterstat{itemp}.bl.avg) * 100 - 100;
                ...
                % plot negative clusters
                if isfield(stats_binned{ipart}.(markername).clusterstat{itemp}, 'negclusters')
                    for ineg = 1 : size(stats_binned{ipart}.(markername).clusterstat{itemp}.negclusters, 2)
                        if stats_binned{ipart}.(markername).clusterstat{itemp}.negclusters(ineg).prob < cfg.stats.alpha
                            sel = find(stats_binned{ipart}.(markername).clusterstat{itemp}.negclusterslabelmat == ineg);
                            bar(stats_binned{ipart}.(markername).clusterstat{itemp}.time(sel), sdf_bar{ipart}.avg(sel+lag), 1, 'facecolor', 'r', 'edgecolor', 'r');
                ...
                % plot baseline
                plot(cfg.stats.(markername).toi, [stats_smooth{ipart}.(markername).clusterstat{itemp}.bl.avg, stats_smooth{ipart}.(markername).clusterstat{itemp}.bl.avg], ':k');

                % plot raster
                subplot(3, 3, [7, 8]); hold;
                cfgtemp                 = [];
                cfgtemp.spikechannel    = itemp;
                cfgtemp.latency         = [cfg.stats.(markername).toi(1), cfg.stats.(markername).toi(2)];
                cfgtemp.trialborders    = 'yes';
                ft_spike_plot_raster(cfgtemp, SpikeTrials{ipart}.(markername));
                ...
                barh = bar(stats_smooth{ipart}.(markername).isi_all.time*1000, stats_smooth{ipart}.(markername).isi_all.avg(itemp, :));
                title('ISI pattern');
                xlabel('ISI (ms)');
                ...
                %             hdr_fname   = fullfile(temp(1).folder, temp(1).name);
                %             hdr         = ft_read_header(hdr_fname); % take the first file to extract the header of the data
                tempsel = squeeze(SpikeRaw{ipart}.template{itemp}(SpikeRaw{ipart}.template_maxchan(itemp)+1, :));
                temptime = ((1:size(SpikeRaw{ipart}.template{itemp}, 3)) / hdr.Fs*1000)';
                ...
                set(fig, 'PaperUnits', 'normalized');
                set(fig, 'PaperPosition', [0 0 1 1]);
                print(fig, '-dpdf', fullfile(cfg.imagesavedir, strcat(cfg.prefix, 'part', num2str(ipart), '-spikerates_template_', num2str(itemp), '_', markername, '.pdf')), '-r600');
                %set(fig, 'PaperOrientation', 'portrait');
                print(fig, '-dpng', fullfile(cfg.imagesavedir, strcat(cfg.prefix, 'part', num2str(ipart), '-spikerates_template_', num2str(itemp), '_', markername, '.png')), '-r600');
                close all
            end % itemplate
        end % ilabel
    end % ipart
