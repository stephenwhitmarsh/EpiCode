function [SpikeRaw, SpikeTrials] = readSpykingCircusSleepStage(cfg,MuseStruct,force,varargin)
...
fname = fullfile(cfg.datasavedir,[cfg.prefix,'spikedataSleepStage.mat']);
...
%             [timestamps, ~, ~] = Nlx2MatCSC_v3(hdr_fname, [0 0 0 1 0]);
        ...
        for hyplabel = {'PHASE_1','PHASE_2','PHASE_3','REM','AWAKE','NO_SCORE'}
            ...
            for idir = 1:length(MuseStruct{ipart})
                ...
                if ~isfield(MuseStruct{ipart}{idir}.markers.([cell2mat(hyplabel),'__START__']),'offset')
                    continue
                end
 
                for i = 1 : size(MuseStruct{ipart}{idir}.markers.([cell2mat(hyplabel),'__START__']).offset,2)
                    y1 = MuseStruct{ipart}{idir}.markers.([cell2mat(hyplabel),'__START__']).offset(i);
                    y2 = MuseStruct{ipart}{idir}.markers.([cell2mat(hyplabel),'__END__']).offset(i);
