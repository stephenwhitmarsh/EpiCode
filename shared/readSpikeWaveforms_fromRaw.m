function [SpikeWaveforms] = readSpikeWaveforms_fromRaw(cfg, SpikeRaw, force)
...
fname = fullfile(cfg.datasavedir, [cfg.prefix, 'spike_waveform_fromraw.mat']);
...
for ipart = cfg.spikewaveform.part_list
    ...
    for icluster = 1 : size(SpikeRaw{ipart}.label, 2)
        ...
        if icluster > 1
            ...
            if ~isempty(cfg.circus.channelname)
                if ~strcmp(SpikeRaw{ipart}.channelname{icluster}, SpikeRaw{ipart}.channelname{icluster-1}); loadnew = true; end
            end
            % if max electrode plot is different from loaded data
            if SpikeRaw{ipart}.template_maxchan(icluster) ~= SpikeRaw{ipart}.template_maxchan(icluster-1); loadnew = true; end
        else
            loadnew = true;
        end
        ...
        if loadnew
            if isempty(cfg.circus.channelname)
                chanindx_cfg    = SpikeRaw{ipart}.template_maxchan(icluster) + 1;
                ...
                channelname     = SpikeRaw{ipart}.channelname{icluster};
                maxchan         = SpikeRaw{ipart}.template_maxchan(icluster);
...
%             cfgtemp.hpfreq                  = cfg.spikewaveform.hpfreq;
        ...
        if strcmp(cfg.spikewaveform.nspikes, 'all')
            spikes_idx_sel = 1:length(SpikeRaw{ipart}.sample{icluster});
        else
            if size(SpikeRaw{ipart}.sample{icluster}, 2) > cfg.spikewaveform.nspikes
                spikes_idx_sel = randperm(length(SpikeRaw{ipart}.sample{icluster}), cfg.spikewaveform.nspikes);
            else
                spikes_idx_sel = 1:length(SpikeRaw{ipart}.sample{icluster});
            end
        end
        ...
        for ispike = spikes_idx_sel
            ...
            Startsample  = [Startsample; round(SpikeRaw{ipart}.sample{icluster}(ispike) + cfg.spikewaveform.toi(1) * hdr.Fs)];
            Endsample    = [Endsample;   round(SpikeRaw{ipart}.sample{icluster}(ispike) + cfg.spikewaveform.toi(2) * hdr.Fs)];
        ...
        SpikeWaveforms{ipart}{icluster}                          = ft_redefinetrial(cfgtemp, chandata);
        SpikeWaveforms{ipart}{icluster}.label                    = [];
        SpikeWaveforms{ipart}{icluster}.label{1}                 = SpikeRaw{ipart}.label{icluster};
        SpikeWaveforms{ipart}{icluster}.template_maxchan         = SpikeRaw{ipart}.template_maxchan(icluster);
        SpikeWaveforms{ipart}{icluster}.trialinfo                = table;
        SpikeWaveforms{ipart}{icluster}.trialinfo.begsample      = Startsample(full_trial);
        SpikeWaveforms{ipart}{icluster}.trialinfo.endsample      = Endsample(full_trial);
        SpikeWaveforms{ipart}{icluster}.trialinfo.offset         = Offset(full_trial);
        SpikeWaveforms{ipart}{icluster}.trialinfo.trialduration  = Endsample(full_trial)-Startsample(full_trial)+1;
        SpikeWaveforms{ipart}{icluster}.trialinfo.trialnr        = Trialnr(full_trial);
        
        %remove cfg field which takes looooot of space on disk
        %(several Go per patient)
        SpikeWaveforms{ipart}{icluster} = rmfield(SpikeWaveforms{ipart}{icluster}, 'cfg');
