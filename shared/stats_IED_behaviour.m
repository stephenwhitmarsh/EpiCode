function [dat_unit, dat_lfp] = stats_IED_behaviour(cfg, MuseStruct, force)
        ...
        for ipart = 1 : size(SpikeTrials_timelocked, 2)
            ...
            for markername = string(fields(LFP{ipart}))'
                ...
                for ilabel = 1 : size(SpikeTrials_timelocked{ipart}.(markername).label, 2)
                    ...
                    temp.cluster_group          = repmat(deblank(string(SpikeTrials_timelocked{ipart}.(markername).cluster_group{ilabel})), size(temp, 1), 1);
...
for ipatient = 1 : size(cfg, 2)
    ...
    [~, hyp, ~]     = hypnogramMuseStats(cfg{ipatient}, MuseStruct{ipatient}, false);
    if isempty(hyp)
        continue
    end
